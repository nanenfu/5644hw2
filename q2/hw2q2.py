...
def hw2q2():
    ...
    plot3(data[0, :], data[1, :], data[2, :], title='Training Data')
    xTrain = data[0:2,:]
    yTrain = data[2,:]
    
    Ntrain = 1000
    data = generateData(Ntrain)
    plot3(data[0, :], data[1, :], data[2, :], col='r', title='Validation Data') 
    # plot3(data[0,:],data[1,:],data[2,:])
    xValidate = data[0:2,:]
    yValidate = data[2,:]
...
def plot3(a, b, c, mark="o", col="b", title="3D Scatter Plot"):
